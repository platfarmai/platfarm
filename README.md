...
**AI 时代的服务农场**：一个 API 网关对外，每个服务一个容器，AI 生成的服务像庄稼一样"种"进平台——`platctl` 一键接入，全程不改平台文件。

## 1. 框架目的

AI 写一个服务只要几分钟，但把它**安全地**放进系统里往往要几天：鉴权怎么接、路由怎么配、数据库怎么隔离、第三方代码敢不敢跑。Platfarm 把这些做成平台纪律，让"产出服务"到"上线服务"之间只剩一条命令。
...
3. **文件是唯一真相源**——网关配置、编排、权限声明全部由 `platctl` 从清单生成并进 git；不存在"后台点一下就变了"的运行态
...
```bash
cd tools/platctl && go build -o platctl.exe . && cd ../..
```

### 2.3 新增一个第一方服务（30 秒）

```bash
./tools/platctl/platctl.exe new svc-hello --lang py   # 模板生成骨架（含验签中间件+契约测试）
./tools/platctl/platctl.exe sync                      # 清单 → 网关配置 + 编排
docker compose up -d --build && docker compose restart gateway
./tools/platctl/platctl.exe check --e2e               # 契约测试三件套闸门
```
...
```bash
./tools/platctl/platctl.exe install ./path/to/plugin/   # 目录含 plugin.yaml（镜像交付）
# 自动完成：校验 → 专属网络 → 插件库开号 → 发凭据 → 起容器 → 契约测试闸门（失败自动 disable）
./tools/platctl/platctl.exe disable|enable|uninstall svc-plugin-x [--purge]
```
...
templates/py-service/         platctl new 骨架
...
1. 生成文件禁止手改；配置变更 = 改清单 + `platctl sync`
...
已落地：auth 底座（RS256+JWKS）、网关验签/限流、platctl 全命令、第三方登录（mock/GitHub）、第三方插件沙箱（专属网络+独立 plugin-pg+凭据体系）、多副本兼容。
路线图（Phase 4~6 余项：响应加密、svc-console 管理台、日志管道等）：architecture-v2.md §9。
