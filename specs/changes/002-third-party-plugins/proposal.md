...
可插拔从"第一方 AI 服务"推向"外部开发者交付的插件"。第三方只信契约不信代码，现有设计有三个洞：HS256 共享密钥可伪造 token、扁平内网可绕过网关、platctl 假设源码在 services/。
...
6. **svc-console**：platctl 的 Web 外壳，唯一持有 docker socket，admin-only
...
- platctl 新增 install/enable/disable/uninstall/upgrade 子命令

## 影响面

- auth（签发/JWKS）、gateway 配置生成、platctl（大改）、templates（验签方式）、存量 svc-demo（验签方式）
...
# 正常路径：
platctl install ./test-plugin && platctl check --e2e         # 安装闸门全绿
docker compose up -d --scale svc-plugin-test=3           # 多副本轮询正常
# 迁移回归：
platctl check --e2e                                        # RS256 切换后存量服务契约不回归
```

## 分阶段（每步独立可合）

RS256+JWKS → 网络分段 → plugin-pg+开号 → platctl 生命周期 → client_credentials+scope 放行 → svc-console
