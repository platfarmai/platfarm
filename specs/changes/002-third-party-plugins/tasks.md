...
## Stage 1 — RS256 + JWKS ✅
- [x] auth：RSA 生成/加载（`.keys/pf-auth.pem`，auth 与 platctl 幂等自举）+ RS256 签发 + JWKS 端点
- [x] platctl sync：Kong consumer 改 `rsa_public_key` 注入
- [x] templates + svc-demo：公钥文件验签（`/pf/jwt.pub` 平台挂载）
- [x] `platctl check --e2e` 回归全绿；JWT_SECRET 概念退役（.env/.env.example 已清）
